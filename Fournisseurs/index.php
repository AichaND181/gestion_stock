...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_fournisseur'])) {
    ...
    if (empty($name)) {
        $response['error'] = "Le nom du fournisseur est requis.";
    } else {
        $stmt = $pdo->prepare("INSERT INTO fournisseurs (name, email, phone, address, status) VALUES (?, ?, ?, ?, ?)");
        if ($stmt->execute([$name, $email, $phone, $address, $status])) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_fournisseur'])) {
    $id = $_POST['id'] ?? 0;
    $name = trim($_POST['name'] ?? '');
    $email = trim($_POST['email'] ?? '');
    $phone = trim($_POST['phone'] ?? '');
    ...
    if (empty($name)) {
        ...
        $stmt = $pdo->prepare("UPDATE fournisseurs SET name = ?, email = ?, phone = ?, address = ?, status = ? WHERE id = ?");
        if ($stmt->execute([$name, $email, $phone, $address, $status, $id])) {
            $response['success'] = true;
        } else {
...
if (isset($_GET['delete_fournisseur'])) {
    $id = $_GET['delete_fournisseur'];
    $stmt = $pdo->prepare("UPDATE fournisseurs SET deleted_at = NOW() WHERE id = ?");
    $stmt->execute([$id]);
    header('Location: index.php?deleted=1');
    exit;
}

// Récupérer un fournisseur pour affichage (AJAX)
if (isset($_GET['action']) && $_GET['action'] == 'get_fournisseur' && isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $pdo->prepare("SELECT * FROM fournisseurs WHERE id = ? AND deleted_at IS NULL");
    $stmt->execute([$id]);
...
// ======================== STATISTIQUES ========================
$stmt = $pdo->query("SELECT COUNT(*) FROM fournisseurs WHERE deleted_at IS NULL");
$totalFournisseurs = $stmt->fetchColumn();

$stmt = $pdo->query("
    SELECT f.id, f.name, COUNT(p.id) as produit_count 
    FROM fournisseurs f 
    LEFT JOIN produits p ON f.id = p.supplier_id AND p.deleted_at IS NULL 
    WHERE f.deleted_at IS NULL 
    GROUP BY f.id
");
$fournisseursStats = $stmt->fetchAll();
...
$countSql = "SELECT COUNT(*) FROM fournisseurs f WHERE f.deleted_at IS NULL";
$countParams = [];

if (!empty($search)) {
    $countSql .= " AND (f.name LIKE :search OR f.email LIKE :search OR f.phone LIKE :search OR f.address LIKE :search)";
...
$totalFournisseursFiltered = $stmtCount->fetchColumn();
$totalPages = ceil($totalFournisseursFiltered / $perPage);

// ======================== REQUÊTE PRINCIPALE ========================
$sql = "SELECT f.*, 
        (SELECT COUNT(*) FROM produits p WHERE p.supplier_id = f.id AND p.deleted_at IS NULL) as produit_count 
        FROM fournisseurs f 
        WHERE f.deleted_at IS NULL";
$params = [];

if (!empty($search)) {
    $sql .= " AND (f.name LIKE :search OR f.email LIKE :search OR f.phone LIKE :search OR f.address LIKE :search)";
...
$sql .= " ORDER BY f.id DESC LIMIT $perPage OFFSET " . (($page - 1) * $perPage);
$stmt = $pdo->prepare($sql);
$stmt->execute($params);
$fournisseurs = $stmt->fetchAll();

// Récupérer les statuts distincts pour le filtre
$stmtStatus = $pdo->query("SELECT DISTINCT status FROM fournisseurs WHERE deleted_at IS NULL AND status IS NOT NULL AND status != ''");
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarMain">
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item"><a class="nav-link active" href="index.php"><i class="fas fa-truck me-1"></i> Fournisseurs</a></li>
...
<div class="container mt-4">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="stat-title">TOTAL FOURNISSEURS</div>
            <div class="stat-number"><?= $totalFournisseurs ?></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon"><i class="fas fa-boxes"></i></div>
            <div class="stat-title">PRODUITS FOURNIS</div>
            <div class="stat-number">
                <?php 
                    $totalProduitsFournis = array_sum(array_column($fournisseursStats, 'produit_count'));
                    echo $totalProduitsFournis;
                ?>
            </div>
        </div>
        <div class="stat-card">
            <div class="stat-icon"><i class="fas fa-chart-line"></i></div>
            <div class="stat-title">MOYENNE PAR FOURNISSEUR</div>
            <div class="stat-number">
                <?= $totalFournisseurs > 0 ? round($totalProduitsFournis / $totalFournisseurs, 1) : 0 ?>
            </div>
        </div>
    </div>
    ...
    <div class="card-table">
        <div class="table-header">
            ...
            <div class="table-header-actions">
                <div class="table-header-info"><i class="fas fa-database"></i> Total : <?= count($fournisseurs) ?> fournisseur(s)</div>
        ...
        <div class="filters-bar">
            <form method="GET" class="d-flex flex-wrap gap-2 w-100 align-items-end">
                <div class="filter-group">
                    ...
                    <input type="text" name="search" class="form-control" placeholder="Nom, email, téléphone..." value="<?= htmlspecialchars($search) ?>">
                </div>
                <div class="filter-group">
        ...
        <div class="table-responsive">
            <table class="table-custom">
                <thead>
                    <tr><th>ID</th><th>Nom</th><th>Email</th><th>Téléphone</th><th>Adresse</th><th>Produits</th><th>Statut</th><th>Actions</th></tr>
                </thead>
                <tbody>
                    <?php foreach ($fournisseurs as $fournisseur): ?>
                    <tr>
                        <td data-label="ID"><?= $fournisseur['id'] ?></td>
                        <td data-label="Nom"><strong><?= htmlspecialchars($fournisseur['name']) ?></strong></td>
                        <td data-label="Email"><?= htmlspecialchars($fournisseur['email'] ?? '—') ?></td>
                        <td data-label="Téléphone"><?= htmlspecialchars($fournisseur['phone'] ?? '—') ?></td>
                        <td data-label="Adresse"><?= htmlspecialchars($fournisseur['address'] ?? '—') ?></td>
                        <td data-label="Produits">
                            <span class="badge" style="background: #4f46e5; color: white; padding: 0.25rem 0.75rem; border-radius: 60px;">
                                <?= $fournisseur['produit_count'] ?? 0 ?> produits
                            </span>
                         </td>
                        <td data-label="Statut">
                            <span class="badge-status" style="background:<?= 
                                match(strtolower($fournisseur['status'] ?? '')) {
                                    'actif' => '#10b981',
                                    'inactif' => '#ef4444',
                                    default => '#6b7280'
                                } ?>;">
                                <?= htmlspecialchars($fournisseur['status'] ?? 'Actif') ?>
                            </span>
                        </td>
                        <td data-label="Actions">
                            <div class="action-icons">
                                <button class="view-action" data-id="<?= $fournisseur['id'] ?>" title="Voir">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button class="edit-action" data-id="<?= $fournisseur['id'] ?>" title="Modifier">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="delete-action" data-id="<?= $fournisseur['id'] ?>" data-name="<?= htmlspecialchars($fournisseur['name']) ?>" title="Supprimer">
                    ...
                    <tr><td colspan="8" class="text-center py-4 text-muted"><i class="fas fa-truck fa-2x mb-2 d-block"></i>Aucun fournisseur trouvé. <a href="#" data-bs-toggle="modal" data-bs-target="#addFournisseurModal">Ajoutez-en un</a>.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
<!-- MODALE AJOUTER FOURNISSEUR -->
<div class="modal fade modal-form" id="addFournisseurModal" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <form id="addFournisseurForm" method="POST">
                    ...
                    <div class="row-2cols">
                        <div>
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-building"></i> Nom *</label>
                        ...
                        <div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="address" class="form-control" rows="3" placeholder="Adresse complète"></textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="add_fournisseur" class="btn-save-modal"><i class="fas fa-save me-2"></i> Enregistrer</button>
                </div>
            </form>
        </div>
    </div>
...
<!-- MODALE VOIR FOURNISSEUR -->
<div class="modal fade modal-form" id="viewFournisseurModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-info-circle me-2"></i> Détails du fournisseur</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
            </div>
            <div class="modal-body" id="viewFournisseurBody">
...
<!-- MODALE MODIFIER FOURNISSEUR -->
<div class="modal fade modal-form" id="editFournisseurModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-pen-alt me-2"></i> Modifier le fournisseur</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
            </div>
            <form id="editFournisseurForm" method="POST">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn-cancel-modal" data-bs-dismiss="modal"><i class="fas fa-times me-2"></i> Annuler</button>
                    <button type="submit" name="edit_fournisseur" class="btn-save-modal"><i class="fas fa-save me-2"></i> Mettre à jour</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade modal-form" id="deleteFournisseurModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Êtes-vous sûr de vouloir supprimer le fournisseur <strong id="deleteFournisseurName"></strong> ?</p>
...
<script>
    ...
    // ========== AJOUTER FOURNISSEUR ==========
    document.getElementById('addFournisseurForm')?.addEventListener('submit', function(e) {
        e.preventDefault();
        const formData = new FormData(this);
        formData.append('add_fournisseur', '1');
        ...
        .then(data => {
            if (data.success) {
                const modal = bootstrap.Modal.getInstance(document.getElementById('addFournisseurModal'));
                modal.hide();
                location.reload();
            } else {
    ...
    document.querySelectorAll('.view-action').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            fetch(`?action=get_fournisseur&id=${id}`)
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        alert(data.error);
                    ...
                    document.getElementById('viewFournisseurBody').innerHTML = `
                        <div class="row-2cols">
                            <div>
                                <div class="detail-item">
                                    <div class="detail-label"><i class="fas fa-building"></i> Nom</div>
                            ...
                            <div>
                                ...
                                <div class="detail-item">
                                    ...
                                    <div class="detail-value">
                                        <span class="badge-status" style="background:${data.status === 'actif' ? '#10b981' : '#ef4444'}">
                                            ${escapeHtml(data.status || 'Actif')}
                                        </span>
                                    </div>
                                </div>
                            </div>
                    ...
                    `;
                    new bootstrap.Modal(document.getElementById('viewFournisseurModal')).show();
                });
        });
    });
    ...
    document.getElementById('editFournisseurForm')?.addEventListener('submit', function(e) {
        e.preventDefault();
        const formData = new FormData(this);
        formData.append('edit_fournisseur', '1');
        ...
        .then(data => {
            if (data.success) {
                const modal = bootstrap.Modal.getInstance(document.getElementById('editFournisseurModal'));
                modal.hide();
                location.reload();
            } else {
                alert('Erreur : ' + data.error);
    ...
    document.querySelectorAll('.delete-action').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('deleteFournisseurName').textContent = this.dataset.name;
            new bootstrap.Modal(document.getElementById('deleteFournisseurModal')).show();
        });
    }); 
    ...
    document.getElementById('confirmDeleteBtn')?.addEventListener('click', function(e) {
        e.preventDefault();
        if (deleteId) {
            window.location.href = `?delete_fournisseur=${deleteId}`;
        }
    });
