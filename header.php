...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_stock'])) {
    ...
    if (empty($sale_number)) {
        $response['error'] = "Le numéro de vente est requis.";
    } else {
        $stmt = $pdo->prepare("
            UPDATE stock 
            SET sale_number = ?, customer_id = ?, subtotal = ?, tax_amount = ?, 
                discount_amount = ?, total_amount = ?, payment_status = ?, 
                payment_method = ?, notes = ? 
            WHERE id = ?
        ");
        if ($stmt->execute([$sale_number, $customer_id, $subtotal, $tax_amount, $discount_amount, $total_amount, $payment_status, $payment_method, $notes, $id])) {
...
// Récupérer la liste des clients pour le formulaire de modification
$stmtClients = $pdo->query("SELECT id, name FROM clients WHERE deleted_at IS NULL ORDER BY name");
                    ...
                    <tr>
                        ...
                        <td data-label="Actions">
                            <div class="action-icons">
                                ...
                                <button class="edit-action" data-id="<?= $entry['id'] ?>" title="Modifier">
...
<div class="modal fade modal-form" id="editStockModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-pen-alt me-2"></i> Modifier l'entrée stock</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
            </div>
            <form id="editStockForm" method="POST">
                    ...
                    <div class="row-2cols">
                        <div>
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-receipt"></i> N° Vente *</label>
                                <input type="text" name="sale_number" id="editSaleNumber" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-user"></i> Client</label>
                                <select name="customer_id" id="editCustomerId" class="form-select">
                            ...
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-money-bill-wave"></i> Sous-total</label>
                                <input type="number" name="subtotal" id="editSubtotal" class="form-control" step="0.01">
                            </div>
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-percent"></i> Taxes</label>
                                <input type="number" name="tax_amount" id="editTaxAmount" class="form-control" step="0.01">
                            </div>
                        </div>
                        <div>
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-tag"></i> Remise</label>
                                <input type="number" name="discount_amount" id="editDiscountAmount" class="form-control" step="0.01">
                            </div>
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-chart-line"></i> Montant total</label>
                                <input type="number" name="total_amount" id="editTotalAmount" class="form-control" step="0.01">
                            </div>
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-truck"></i> Statut</label>
                            ...
                            <div class="form-group">
                                <label class="form-label"><i class="fas fa-credit-card"></i> Mode de paiement</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="edit_stock" class="btn-save-modal"><i class="fas fa-save me-2"></i> Mettre à jour</button>
                </div>
            </form>
        </div>
    </div>
        ...
        btn.addEventListener('click', function() {
            ...
            fetch(`?action=get_stock&id=${id}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('editSaleNumber').value = data.sale_number;
                    document.getElementById('editCustomerId').value = data.customer_id || 0;
                    document.getElementById('editSubtotal').value = data.subtotal;
                    document.getElementById('editTaxAmount').value = data.tax_amount;
